...
boolean busy = false;
void setup(){
  lidar = new Serial(this, "COM4");//parity=none databits=8 stopbits=1 baudrate = 9600 are defaults
  lidar.buffer(2);
  size(600,300);
  screen();
}
...
void keyTyped(){
  if(key == '1' && busy == false){//100 steps
    lidar.write(0x00);
    steps = 100;
    degperstep = 180.0/steps;
  }
  else if(key == '2' && busy == false){//200 steps
    lidar.write(0x01);
    steps = 200;
    degperstep = 180.0/steps;
  }
  else if(key == '3' && busy == false){//400 steps
    lidar.write(0x02);
    steps = 400;
    degperstep = 180.0/steps;
  }
  else if(key == '4' && busy == false){//move motor CCW
    lidar.write(0x07);
  }
  else if(key == '5' && busy == false){//turn motors off
    lidar.write(0x05);
  }
  else if(key == '6' && busy == false){//move motor CW
    lidar.write(0x06);
  }
  else if(key == '7'){//scan once
    lidar.write(0x04);
  }
  else if(key == '8'){
